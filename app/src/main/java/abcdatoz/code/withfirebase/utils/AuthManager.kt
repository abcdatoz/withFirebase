package abcdatoz.code.withfirebase.utils
...
sealed class AuthRes<out T> {
    ...
    data class Error<T>(val errorMessage: String) : AuthRes<T>()
}
...
class AuthManager(private val context: Context) {
    ...
    suspend fun signInAnonymously(): AuthRes<FirebaseUser> {
        return try {
            ...
            Success(result.user ?: throw Exception("Error al iniciar sesion"))

        } catch (e: Exception) {
            Error(e.message ?: "Error la iniciar sesion")
        }
    }
    ...
    suspend fun resetPassword(email: String): AuthRes<Unit> {
        return try {
            ...
            AuthRes.Error(e.message ?: "Error al restablecer la contrase;a")
        }
    }
